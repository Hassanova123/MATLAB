...
nStep = 5000; % Number of time levels
L = 2.0; % Domain length
...
%  for j = 1:N
%  x = dx*(j-1);
%  exact(j) = exp(-(0.75*exp(-((x(i)-0.5)/0.1).^2))^2 * D * time) *... ; % Exact solution of the current time level
%  end
 ...
 title(sprintf('time= %1.2f',time),'fontsize',16)
